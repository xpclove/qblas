...
//   Step 5: q_qsvt → q_qsvt_normalize_vector    → Vector normalization
//   Step 6: q_qsvt → q_qsvt_check_dims          → Dimension check
//   Step 7: q_qsp → q_qsp_apply_rotation        → Quantum rotation
//
// Verification:
//   - symmetric_phase_seq(4): length == 9 (Fact)
//   - chebyshev_phase(3): length == 4 (Fact)
//   - poly_eval([1,0.5,0.25], 0.5) = 1.3125 (Fact)
//   - is_valid_phase([0.5]): true (Fact)
//   - normalize_vector([3,4])[0] = 0.6, [1] = 0.8 (Fact)
//   - check_dims(1, 1): true (Fact)
//
