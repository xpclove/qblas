...
// ============================================================
    ...
    // sF rotation with independent sign qubit (sign separate from weight)
    operation q_walk_simulation_sF_sign(
        qs_weight : Qubit[], qs_sign : Qubit,
        t : Double, n_bits_float : Int, Rp : Pauli
    ) : Unit is Adj + Ctl {
        let nbit = Length(qs_weight);
        for i in 0 .. nbit - 1 {
            let fi = IntAsDouble(i);
    ...
    // T gate with sF rotation, controlled version, sign qubit independent
    operation q_walk_simulation_C_T_R_sF_sign(
        qs_controls : Qubit[], Rp : Pauli,
        qs_a : Qubit[], qs_b : Qubit[], qs_sign : Qubit,
        qs_weight : Qubit[], n_bits_float : Int, t : Double
    ) : Unit {
        q_walk_op_A(qs_a, qs_b, qs_sign);
        (Controlled q_walk_simulation_sF_sign)(qs_controls,
            (qs_weight, qs_sign, t, n_bits_float, Rp));
        (Adjoint q_walk_op_A)(qs_a, qs_b, qs_sign);
    }
    ...
    // T gate with R + sF rotation (float-weighted), sign qubit independent
    operation q_walk_simulation_T_R_sF_sign(
        Rp : Pauli, qs_a : Qubit[], qs_b : Qubit[], qs_sign : Qubit,
        qs_weight : Qubit[], n_bits_float : Int, t : Double
    ) : Unit {
        q_walk_op_A(qs_a, qs_b, qs_sign);
        q_walk_simulation_sF_sign(qs_weight, qs_sign, t, n_bits_float, Rp);
        (Adjoint q_walk_op_A)(qs_a, qs_b, qs_sign);
    }
    ...
    operation q_walk_simulation_matrix_1_sparse_core(matrix_type : Int, matrix_A : q_matrix_1_sparse_oracle, qs_state : Qubit[], t : Double) : Unit {
        ...
        // Sign qubit is allocated separately so oracle encoding/decoding
        // (O / O†) doesn't interfere with the walk operator's sign rotation.
        ...
        q_walk_simulation_T_R_sF_sign(Rp, qs_a, qs_b, qs_sign, qs_weight, nbit_float, time);
    ...
    operation q_walk_simulation_C_matrix_1_sparse_core(qs_controls : Qubit[], matrix_type : Int, matrix_A : q_matrix_1_sparse_oracle, qs_state : Qubit[], t : Double) : Unit {
        ...
        let time = t_sign * t;
        q_walk_op_M(matrix_A, qs_a, qs_b, qs_weight);
        q_walk_simulation_C_T_R_sF_sign(qs_controls, Rp, qs_a, qs_b, qs_sign, qs_weight, nbit_float, time);
        (Adjoint q_walk_op_M)(matrix_A, qs_a, qs_b, qs_weight);
        Reset(qs_sign);
    }
